...
def Write2txt_IntergrationPointsFromQuesoConditionsWithWeight(condition, filename):
    IntegrationPoints = []
    condition_settings = condition.GetSettings()
    condition_id = condition_settings.GetInt("condition_id")
    for condition_segment in condition.GetSegments():
        ...
        for tri_id in range(num_triangles):
            ...
            global_points = triangle_mesh_segment.GetIntegrationPointsGlobal(tri_id, integration_method)
            for point in global_points:
                IntegrationPoints.append([point[0],point[1],point[2],point.Weight()])
    
    # Write to .txt file
    with open(filename, 'w') as file:
        for point in IntegrationPoints:
            file.write(f"Point: {point[0]}, {point[1]}, {point[2]}, Weight: {point[3]}\n")
